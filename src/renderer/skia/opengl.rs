use std::{ffi::CString, num::NonZeroU32, time::{Duration, Instant}};
...
use crate::backend::{windows::{BackendWindow, BackendWindows}, Backends};

use super::SkiaRenderingBackend;


pub struct OpenGLSkiaBackend {

}

impl SkiaRenderingBackend for OpenGLSkiaBackend {
    fn create_context(backend: Backends, window: BackendWindows) {
...
/*
    ...
    let gl = unsafe { glow::Context::from_loader_function(|a| glfw.get_proc_address_raw(a)) };
    gl::load_with(|a| glfw.get_proc_address_raw(a));
    
    let interface = skia_safe::gpu::gl::Interface::new_load_with(|name| {
        if name == "eglGetCurrentDisplay" {
            return std::ptr::null();
        }
        glfw.get_proc_address_raw(name)
    }).unwrap();
    ...
    let num_samples = glow::NUM_SAMPLE_COUNTS;
    let stencil_size = glow::TEXTURE_STENCIL_SIZE;

    create_surface((f_w, f_h), fb_info, &mut gr_context, 1, 1);
    ...
    while !window.should_close() {
        ...
        unsafe {
            gl.clear_color(1.0, 0.5, 0.1, 0.1);
...
fn create_surface(
    ...
    let backend_render_target =
        skia_safe::gpu::backend_render_targets::make_gl(size, num_samples, stencil_size, fb_info);
